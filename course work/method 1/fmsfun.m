...
global t1 t2 T t x x_start x_end x3f
t1 = switchingTimesArray(1); t2 = switchingTimesArray(2) ; T = switchingTimesArray(3);
[t,x]=ode23s('odefun',[0 T], x_start);
discrepancy = (x(end,1)-x_end(1))^2 + (x(end,2)-x_end(2))^2 + (x(end,3)-x3f)^2;
subplot(3, 1, 1), plot(t, x); title('ѕереходные процессы x1(t), x2(t), x3(t)')
subplot(3, 1, 2), plot(x(:,2), x(:,1), 'b'); title('‘азова€ плоскость x1(x2)')
